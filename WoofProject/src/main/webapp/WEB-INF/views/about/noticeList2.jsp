...
<script type="text/javascript">
	$(document).ready(function() {
		...
		$("#ajaxput").on("click",function(){
			...
			$.ajax({
				type : "put",
				/* url : "/notice/getNoticeListAjaxPutPara"+"?"+"searchCondition"+$("#searchCondition").val()+"&"+"searchKeyword"+$("#searchKeyword").val(), */
				...
				success : function(result){
					...
					for (var i = 0;i<result.length;i++){
						...
						noticeList += '<td class=" text-center p-1" style="width: 50px;">'+result[i].noticeNo+'</td>';
						noticeList += '<td class="text-truncate p-1"><a	href="/notice/getNotice/'+result[i].noticeNo+'"	class="list-group-item list-group-item-action border-0 text-truncate">'+result[i].noticeTitle+'</a></td>';
						noticeList += '<td class=" text-center p-1" style="width: 130px;">'+result[i].noticeRegDate+'</td>';
						noticeList += '</tr>';
					}
		...
		$("#ajaxgetPara").on("click",function(){
				...
				$.ajax({
					type : "get",
					/* url : "/notice/getNoticeListAjaxPutPara/"+$("#searchKeyword").val(), */
					/* url : "/notice/getNoticeListAjaxPutPara"+"?searchCondition="+$("#searchCondition").val()+"&searchKeyword="+$("#searchKeyword").val(), */
					url : "/notice/getNoticeListAjaxPutPara",
					/* data : JSON.stringify(NoticeSearch), */
					data : NoticeSearch,
					/* contentType : "application/json; charset=UTF-8", */
					success : function(result){
						...
						$("#noticeListSpan").html(noticeList);
						let url ="?searchCondition="+$("#searchCondition").val()+"&searchKeyword="+$("#searchKeyword").val();	
